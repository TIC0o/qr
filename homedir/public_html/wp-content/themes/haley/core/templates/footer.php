...
if (!function_exists('haley_socials_function')) {

	function haley_socials_function() {
		...
		$socials = array ( 
			...
			"x" => array( "icon" => "icon-twitter-x" , "target" => "_blank"),
		...
		foreach ( $socials as $name => $attrs) { 
			...
			if (haley_setting('haley_footer_'.$name.'_button')): 

				$i++;
				
				$link = 	esc_url(haley_setting('haley_footer_'.$name.'_button'), array( 'http', 'https', 'tel', 'skype', 'mailto' ) );
		...
		if ( haley_setting('haley_footer_rss_button') == "on" ): 
	...
	add_action( 'haley_socials', 'haley_socials_function' );
